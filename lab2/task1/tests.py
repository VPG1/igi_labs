...
def test_count_sentences_0():
    text = ''
    assert main.number_of_sentences(text) == 0


def test_count_sentences_1():
    text = 'Hi, Mr. Tom.'
    assert main.number_of_sentences(text) == 1
...
def test_count_sentences_3():
    ...
    assert main.number_of_sentences(text) == 2
...
def test_count_sentences_10():
    ...
    assert main.number_of_sentences(text) == 3


def test_count_sentences_11():
    text = 'Hi, Mr. Tom. Qwe e.g., qwe! Qwe e.g., fef.'
    assert main.number_of_sentences(text) == 3
...
def test_nondecl_sentences_0():
    text = ''
    assert main.number_of_non_declaration_sentences(text) == 0


def test_nondecl_sentences_1():
    text = 'Hi, Mr. Tom. Goodbye, Mrs. Smith?!'
    assert main.number_of_non_declaration_sentences(text) == 1
...
def test_nondecl_sentences_4():
    ...
    assert main.number_of_non_declaration_sentences(text) == 2
...
def test_avg_sen_len_3():
    ...
    exp = (7 + 15 + 15 + 16) / 4
    assert main.average_length_of_sentences(text) == exp
...
def test_avg_word_len_1():
    text = ''
    assert main.average_length_of_words(text) == 0


def test_avg_word_len_2():
    text = 'Hi, Mr. Tom.'
    exp = (2 + 2 + 3) / 3
    assert main.average_length_of_words(text) == round(exp, 2)
...
def test_top_k_repeated_ngrams_6():
    text = 'Hi hi Hi hi 123 123 123 123 123 123 123 123 123 123'
    exp = [('Hi hi', 2), ('hi Hi', 1)]
